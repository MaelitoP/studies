...
public class Caractere extends Forme
{
    ...
    protected int valeurCar;
    protected byte[] tabCar;
    protected String[] tabCarString = new String[12];
    ...
    public Caractere(int x, int y, char caractere)
    {
        ...
        this.valeurCar = caractere; // int avec la valeur ASCII du caractere
        this.tabCar = images[this.valeurCar];
        this.init();
    }
    ...
    public void init()
    {
        for(int i = 0; i < this.getxLength(); i++){
            ...
            // on transforme le byte en string qui donne sa valeur decimale en
            // string
            String str = Byte.toString(tabCar[i]);

            // on prend sa valeur en integer
            int valeurStr = Integer.parseInt(str);

            // on prend sa valeur en string en binaire
            String valeurBinaireStr = Integer.toBinaryString(valeurStr);

            // on rajoute les 0 aux debut si necessaire
            valeurBinaireStr = ajouter0(valeurBinaireStr);

            // on place dans tableau du caracter en strings
            this.tabCarString[i] = valeurBinaireStr;
        }

        for(int i = 0; i < this.getxLength(); i++){ // jusqua 12
            for(int j = 0; j < this.getyLength(); j++){

                if(this.tabCarString[i].charAt(j) == '1'){
                    (super.getShape())[i][j] = super.getChar();
                }
            }
        }
    }

    public String ajouter0(String strBinare)
    {
        StringBuilder str = new StringBuilder(strBinare);
